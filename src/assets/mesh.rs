...
impl Default for SyncMeshLoader {
    fn default() -> Self {
...
impl Loader<Mesh> for SyncMeshLoader {
    fn load(&self, ctx: &mut GlfwSurface, asset_name: &str) -> Asset<Mesh> {
            ...
            .and_then(|buf| bincode::deserialize::<RawMesh>(&buf).map_err(AssetError::BincodeError))
        {
            Ok(meshLoaded) => {
                ...
                for p in meshLoaded.primitives {
                    ...
                    let mut tess_builder = TessBuilder::new(ctx).add_vertices(vertices);
                    ...
                    primitives.push(Primitive {
                        tess: tess_builder.build().unwrap(),
                        material: Some(0), // FIXME
                    })
                }
