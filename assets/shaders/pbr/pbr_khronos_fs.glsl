...
//
//    #ifdef HAS_NORMAL_TEXTURE
//    vec2 normal_uv = v_UV[u_normal_tex_coord];
//    vec3 n = texture(u_normal_sampler, normal_uv).rgb;
...
//    n *= (2.0 * float(gl_FrontFacing) - 1.0);
        ...
        vec2 normal_uv = v_UV[u_NormalTexCoord];
        vec3 tangentNormal = texture(u_NormalSampler, normal_uv).xyz * 2.0 - 1.0;
